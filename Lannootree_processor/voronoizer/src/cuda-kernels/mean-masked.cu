...
__global__ void mean_of_mask(uchar3* d_input_image, u_int16_t* d_mask, uint3* d_total, uint* d_count, int i, size_t rows, size_t cols) {
  size_t y = blockIdx.y * blockDim.y + threadIdx.y;
  size_t x = blockIdx.x * blockDim.x + threadIdx.x;

  if (x >= rows || y >= cols || (d_mask[x * cols + y] != (i))) return;
  
  uchar3 color = d_input_image[x * cols + y];

  atomicAdd(&d_total[i].x, color.x);
  atomicAdd(&d_total[i].y, color.y);
  atomicAdd(&d_total[i].z, color.z);
...
void mean_of_mask_launcher(uchar3* d_input_image, u_int16_t* d_mask, uint3* d_total, uint* d_count, int i, size_t rows, size_t cols) {
  ...
  mean_of_mask<<<grid_size, block_size>>>(d_input_image, d_mask, d_total, d_count, i, rows, cols);
}
